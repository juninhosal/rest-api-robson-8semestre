<?php

echo "Nome: Paulo de Tarso Salgado Junior<br />";
echo "R.A.: D301EH-1<br />";
echo "Instituição: UNIP - Bauru<br />";
echo "Curso: Ciência da Computação<br />";
echo "Série: 8ª<br /><br />";

echo "URL Git: https://github.com/juninhosal/rest-api-robson-8semestre<br />";
echo "URL Heroku: https://dsdtrabalho-paulotarso.herokuapp.com<br /><br />";

echo "O método GET foi utilizado na criação de todas as requests do trabalho<br />";
echo "Para baixar a Collection Postman, utilize o END Point /???????.json";